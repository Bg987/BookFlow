...
exports.addLibrary = async (req, res) => {
  ...
  try {
    ...
    const verificationExpire = new Date(Date.now() + 1000*24 * 60 * 60 * 15); // 15 days
    ...
    res.status(201).jon({
      message:
        "Library created successfully. Verification email sent. Verify Account Then you can login",
    });
